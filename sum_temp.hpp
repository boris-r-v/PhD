...
namespace Sci
{
    ...
    *		cтепень серости поверхности корпуса ДТ(body_grayness)	
    ...
    */
	...
	public:
	    dt()=delete;
	    ~dt()=default;
	    dt( double coil_M, double coil_HC, double coil_R, double coil_CSA, double coil_L, double coil_S,
    ...
    */
    ...
    {
	    double h_coil_oil_, h_oil_body_, h_oil_core_, h_body_air_, h_body_ground_, sun_radiation_, external_temp_;
	public:
	    heat_transfer_data()=delete;
	    ~heat_transfer_data()=default;
	    heat_transfer_data(double h_coil_oil, double h_oil_body, double h_oil_core, double h_body_air, double h_body_ground, double sun_radiation, double external_temp );
	    ...
	    void h_coil_to_metal_calc( double Toil );
	    void h_body_to_air( double Tbody, double Tair );

    };

    class heat_condition
    {
	    double oil_temp_, coil_temp_, core_temp_, body_temp_, current_;
	public:
	    heat_condition() = delete;
	    ~heat_condition() = default;
	    heat_condition( double oil_temp, double coil_temp, double core_temp, double body_temp );
    ...
    */
	...
	public:
	    ...
	    dt_heat_transfer( dt const& dt, heat_transfer_data& htd, heat_condition& ic );

	    /**
	    * Метод реализующий расчет
	    * 	current - текущее значение эффективного тока
	    * 	calculating_time - время рассчета в минутах
    	    */	    
	    void calc( double current, double calculating_time );
	
	    /**
	    * Получить абсолютные температы элементов
	    */
	    inline double oil_temp() const 	{return hc_.oil_temp() - 273; }
	    inline double coil_temp() const 	{return hc_.coil_temp() - 273; }
	    inline double core_temp() const 	{return hc_.core_temp() - 273; }
	    inline double body_temp() const 	{return hc_.body_temp() - 273; }

	    inline double oil_temp_K() const 	{return hc_.oil_temp(); }
	    inline double coil_temp_K() const 	{return hc_.coil_temp(); }
	    inline double core_temp_K() const 	{return hc_.core_temp(); }
	    inline double body_temp_K() const 	{return hc_.body_temp(); }
